// 使用 Fetch 获取商品，并将商品传递给 initialize()
// 报告 Fetch 操作中出现的错误
// 商品成功加载并通过 response.json() 转换为 JSON 对象后，
// 调用 initialize() 函数
fetch('products.json')
  .then( response => {
    if (!response.ok) {
...
function initialize(products) {
  ...
  // fetchBlob() 使用 Fetch 获取商品图片，
  // 然后将 Blob 和商品对象传递给 showProduct() 显示
  function fetchBlob(product) {
    ...
    fetch(url)
      ...
      .catch( err => console.error(`获取商品图片失败：${err.message}`) );
  }
